...
def main():
    parser = argparse.ArgumentParser(description="Convert Unified_PPI JSON to PubTator format")
    parser.add_argument('--input_file', required=True, help="Path to input Unified_PPI JSON Lines file")
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            import hashlib
            base_id = item['id'].rsplit('.p', 1)[0]
            # Format base_id to look like 'AIMed_d193_1641' if possible
        ...
        for doc_id, items in documents.items():
            ...
            for item in items:
                for rel in item.get('relation', []):
                    ...
                    # Record positive relations
                    if rel.get('relation_type') == 'positive':
            ...
            for r in unique_relations:
                out_f.write(f"{doc_id}\tAssociation\t{r[0]}\t{r[1]}\n")
